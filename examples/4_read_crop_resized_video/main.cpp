...
void transcode(ffmpegcv::VideoCapture& cap, ffmpegcv::VideoWriter& writer) {
    while (cap.isOpened()) {
...
int main(int argc, char* argv[]) {
    ...
    // 2. resize
    ffmpegcv::VideoCapture cap2("../input.mp4",
        "bgr24",
        {0,0,0, 0},  // crop_xywh, {0,0,0,0} for no crop
        {500, 300}   // resize_wh, {0,0} for no resize
    );

    ffmpegcv::VideoWriter writer2("../output_resize.mp4", 
        "h264",                   // codec
        cap2.fps,                  // frame rate
